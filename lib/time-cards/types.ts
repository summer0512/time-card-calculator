...
export const savedBreakSchema = z.object({
  kind: z.enum(["break", "lunch"]),
  position: z.number().int().nonnegative(),
  minutes: z.number().int().nonnegative(),
});

export const savedRowSchema = z.object({
  position: z.number().int().nonnegative(),
  workDate: z.string().date().nullable().optional(),
  dayLabel: z.string().max(160),
  punches: z.array(punchSchema).max(24),
  breaks: z.array(savedBreakSchema).max(24),
});

const multiplierTierSchema = z.object({
  id: z.string().min(1).max(100),
  afterHours: z.number().nonnegative(),
  rateType: z.literal("multiplier"),
...
export const settingsSchema = z.object({
  mode: z.enum(["time-card", "hours", "split-shift"]),
  timeFormat: z.enum(["auto", "12h", "24h", "military"]),
  showLunchColumn: z.boolean(),
  breakColumnCount: z.number().int().min(0).max(24),
  showBreakDeduction: z.boolean(),
  isBiweekly: z.boolean(),
  copyVariant: z.enum(["time-card", "timesheet", "time-clock", "punch"]),
  overtime: z.object({
    enabled: z.boolean(),
    basis: z.enum(["weekly", "daily"]),
    tiers: z.array(z.discriminatedUnion("rateType", [
      multiplierTierSchema,
      fixedTierSchema,
    ])).max(12),
  }),
});
...
export const timeCardInputSchema = z.object({
  ...
  reportHeader: z.string().max(500).default(""),
  notes: z.string().max(5000).default(""),
  calculatorType: z.string().min(1).max(120),
  sourcePath: z.string().startsWith("/").max(500).refine(
